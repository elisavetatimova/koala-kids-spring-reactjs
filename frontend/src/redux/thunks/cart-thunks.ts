...
export const fetchCart = (data: Array<number>) => async (dispatch: Dispatch) => {
    ...
    const products: Map<number, number> = new Map(JSON.parse(<string>localStorage.getItem("products")));
    let total: number = 0;

    products.forEach((value: number, key: number) => {
        const product: Product = response.data.find((product: { id: number; }) => product.id === key);
...
export const calculateCartPrice = (products: Array<Product> | any) => (dispatch: Dispatch) => {
    ...
    productsFromLocalStorage.forEach((value: number, key: number) => {
        const product: Product = products.find((product: { id: number; }) => product.id === key);
        total += product.price * value;
    });
    dispatch(calculateCartPriceSuccess(total));
};
