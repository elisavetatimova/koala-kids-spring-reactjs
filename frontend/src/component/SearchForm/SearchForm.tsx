...
const SearchForm: FC<PropsType> = ({data, searchByData, setFilteredData, setSearching}) => {
    ...
    const submitHandler = (event: FormEvent<HTMLFormElement>): void => {
        ...
        if (search.trim() !== '') {
            ...
            const filtered: Array<Product> = copiedData.filter((producer: any) => {
                let searchKey: string = 'producer';
                if (searchByData && searchByData.length > 0) {
                    searchKey = searchBy;
                }
                return producer[searchKey].toLowerCase().includes(search.trim().toLowerCase());
            });
